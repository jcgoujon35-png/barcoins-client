  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-6" style={{background:'linear-gradient(160deg, #1a1a2e 0%, #16213e 60%, #0f3460 100%)'}}>
      ...
      <div className="w-full max-w-sm slide-up">
        ...
        <h1 className="text-white text-xl font-bold mb-1">Rejoindre la soirée</h1>
        <p className="text-white/50 text-sm mb-8">Le Bar des Amis — Narbonne</p>
        {step === 'phone' ? (
          <>
            <label className="text-white/60 text-xs uppercase tracking-wider mb-2 block">Ton numéro de téléphone</label>
            <input
              type="tel" value={phone} onChange={e => setPhone(e.target.value)}
              placeholder="06 XX XX XX XX"
              className="w-full px-4 py-3 rounded-xl text-white text-lg mb-4 outline-none"
            ...
            <p className="text-white/30 text-xs text-center mt-4">Déjà inscrit ? Tu seras reconnu automatiquement.</p>
          </>
        ) : (
          <>
            <label className="text-white/60 text-xs uppercase tracking-wider mb-2 block">Code reçu par SMS</label>
            <input
              type="number" value={otp} onChange={e => setOtp(e.target.value)}
              placeholder="• • • •"
              className="w-full px-4 py-3 rounded-xl text-white text-2xl text-center tracking-[1rem] mb-4 outline-none"
              style={{background:'rgba(255,255,255,0.1)', border:'1px solid rgba(255,255,255,0.2)'}}
            />
            ...
            <button onClick={() => setStep('phone')} className="w-full text-center text-white/40 text-sm mt-3">Changer de numéro</button>
          </>
        )}
      </div>
    </div>
