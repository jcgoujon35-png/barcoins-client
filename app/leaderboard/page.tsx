    ...
    <div className="min-h-dvh pb-20" style={{ background: 'linear-gradient(180deg, #1a0a00 0%, #2d1200 50%, #1a0a00 100%)' }}>
      ...
      {gapTo1st <= 0 && me && (
        <div className="mx-4 mt-3 rounded-xl px-4 py-2 flex items-center gap-2"
          ...
          <span className="text-lg">🥇</span>
          <span className="text-sm font-bold" style={{ color: '#10B981' }}>Tu mènes la soirée ! Continue comme ça !</span>
        </div>
      )}
        ...
        {[top3[1], top3[0], top3[2]].map((p, i) => {
          ...
          return (
            <div key={i} className="flex flex-col items-center gap-2">
              ...
              <div
                className={`${sizes[i]} aspect-square rounded-full flex items-center justify-center font-bold text-sm`}
                style={{
                  background: p?.isMe
                    ? 'linear-gradient(135deg, #F59E0B, #FF6B35)'
                    : isCenter ? 'rgba(245,158,11,0.25)' : 'rgba(255,255,255,0.1)',
                  color: p?.isMe ? '#1a0a00' : '#F5E6D3',
                  boxShadow: p?.isMe ? '0 0 16px rgba(245,158,11,0.5)' : isCenter ? '0 0 12px rgba(245,158,11,0.3)' : 'none',
                  border: isCenter ? '1.5px solid rgba(245,158,11,0.5)' : '1px solid rgba(255,255,255,0.1)',
                }}>
                {p?.initials}
              </div>
              <div
        ...
        {rest.map((p) => (
          <div key={p.rank} className="flex items-center gap-3 px-4 py-3 border-b"
            style={{
              ...
              background: p.isMe ? 'rgba(245,158,11,0.12)' : 'transparent'
            }}>
            <span className="text-[#A07850] w-5 text-sm font-bold">{p.rank}</span>
            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
              style={{
                background: p.isMe ? 'linear-gradient(135deg, #F59E0B, #FF6B35)' : 'rgba(255,255,255,0.1)',
                color: p.isMe ? '#1a0a00' : '#F5E6D3',
              }}>
              {p.initials}
            </div>
            <span className="flex-1 text-sm font-medium" style={{ color: p.isMe ? '#F59E0B' : '#F5E6D3' }}>
              {p.name}{p.isMe ? ' (toi)' : ''}
            </span>
            <span className="font-bold text-sm" style={{ color: '#F59E0B' }}>{p.coins}</span>
            <span className="text-xs w-10 text-right" style={{ color: p.delta >= 0 ? '#10B981' : '#ef4444' }}>
